...
/**
 * A class which removes the optional runnable parameter from cyclic barrier method creation expressions in a compilation unit
 */
public class MBRModifier extends MutationModifier {
    /**
     * A method which walks through all the object creations expressions in a compilation unit
     * If the object creation expression is a cyclicbarrer with an optional runnable parameter
     * and marked as the current concurrency object for mutation, the optional runnable parameter is removed
     *
     * The line reference and method name of the semaphore mutant is set in the
     * MutationModifier superclass.
     *
     * If the current object creation expression is a cyclicbarrier
        ...
        if (name.equals("CyclicBarrier")){
            if (oc.getChildNodes().size()==3) {
                if (getMutationNumber()==getMutationsVisited()){
                    ...
                    oc.remove(oc.getChildNodes().get(2));
                }
